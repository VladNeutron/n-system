<template>
                ...
                <table class="table table-hover text-wrap text-start table-striped">
                  <thead>
                    <tr class="text-start">
                      ...
                      <th scope="col" class="th__col">Накладная</th>
                    ...
                    <tr v-for="order of paginationList" :key="order.id">
                      ...
                      <td class="dropdown" style="border-left: 0; width: 100%">
                        ...
                        <div class="dropdown-content">
                          <a href="/accounting/documents/edit-account">Редактировать</a>
                          <hr />
  ...
  <the-filter @no-filter="cancelFilters">
    ...
    <p class="text-start my-2 fw-bold" for="наименование документа">
      Наименование документа
    </p>
  ...
  <delete-modal :title="'счета'" :text="`документ  &quot;счет №12132145&quot;`"></delete-modal>
</template>
    ...
    filteredOrders() {
      return this.orders.filter(
        (order) =>
          ...
          (String(order.note).toLowerCase().includes(String(this.search).toLowerCase())) ||
          (String(order.id).toLowerCase().includes(String(this.search).toLowerCase()))
      );
    },
  },
